...
**계약 적용 범위 — 3원칙 (그쪽 프로젝트 분석과 상호 확인됨, §9와 동일):**
- [ ] **① 쓰기+읽기 양쪽**: setAngle/commit(쓰는 쪽)만이 아니라 **각도를 읽어 계산하는 쪽 — guide·bbox·group angle 계산 — 도 같은 규칙**을 쓴다. 한쪽만 바꾸면 저장값은 맞아도 화면 계산이 어긋난다.
- [ ] **② 규칙 통합이지 함수 통합이 아님**: 경로·함수가 여러 개여도 **같은 각도 계약**만 쓰면 된다. 공통 함수 1개로 합치라는 뜻이 아니다.
- [ ] **③ 통일 대상은 배치 회전(placement) 계약뿐**: arc/ellipse의 내부 geometry 의미(start/sweep·축)는 **도형별로 따로 보존**한다. 모든 각도를 하나의 수치로 밀면 깨진다.
...
## 9. 오독 방지 — 표현 정정 3가지 (✅ 그쪽 프로젝트 분석과 합의 확인됨)

> 위 표현을 **문자 그대로 좁게 읽으면** 잘못 적용된다. 의도를 정정한다.
> **그쪽 프로젝트 AI의 독립 분석도 동일한 3가지를 지적** — 양쪽 해석이 일치함을 확인했다. 같은 내용을 §2 본문 "계약 적용 범위 3원칙"에도 반영해 두었으니, 이 문서를 분석에 쓸 때 §2만 읽어도 원칙이 전달된다.
