# 그룹 도형 × 선택상자 디버깅 가이드 (질문지 + 레퍼런스 체크리스트)

> **이 문서 하나로 끝낸다.** 구성은 3부.
> - **Part 1 — 질문지**: 그룹 도형 프로젝트 AI에게 그대로 붙여넣어 "지금 어떻게 구현돼 있는지" 답을 받는다.
> - **Part 2 — 답 해석 가이드**: 받은 답을 어디로 수정할지 매핑한다.
> - **Part 3 — 레퍼런스 구현 체크리스트**: 이 저장소(`map-aabb`)의 올바른 규약. 답을 대조할 기준.
>
> 이 저장소(`map-aabb`)는 "선택상자(SVG 오버레이 + 핸들)"로 도형을 이동·회전·정점편집하고,
> 같은 데이터를 Three.js로 3D extrude 하는 **레퍼런스 구현**이다. 핵심은 전부 `map-aabb/src/app/app.tsx` 한 파일.
> 분석 대상(그룹 도형 프로젝트)은 이 규약을 여러 도형으로 확장한 것이라고 보고, 어디서 규약이 깨졌는지 찾는다.

---

## 전제: 두 프로젝트는 동일 환경

이 가이드는 **두 프로젝트(레퍼런스 = 이 저장소, 분석 대상 = 그룹 도형 프로젝트)가 아래와 동일한 스택**이라는 전제로 작성됐다. 좌표계/각도/Y부호 규약은 이 스택 기준이다. 그룹 프로젝트의 버전이 다르면 그 항목부터 의심한다.

| 패키지 | 버전 | 규약상 중요한 이유 |
|---|---|---|
| `maplibre-gl` | **4.7.1** | custom layer `render(gl, matrix)` 시그니처(v4 기준). `MercatorCoordinate` / `meterInMercatorCoordinateUnits()` / `project`·`unproject` 좌표 규약. **Mercator Y가 아래로(+)** → Y부호 반전·각도 처리의 근거. (별도 `@types/maplibre-gl` 없이 패키지 내장 타입 사용) |
| `three` | **0.174.0** | `ExtrudeGeometry`, `Matrix4`, `MathUtils.degToRad/radToDeg`, `Group.rotation.z`, `LineDashedMaterial`. |
| `react` / `react-dom` | **19.x** (런타임) | `useRef`/`useState` 이중화로 stale closure 회피(Part 3 / 10장). |
| `@types/react` | **18.2.x** (`^18.2`) | 타입은 18 계열로 고정. 런타임(19)과 메이저가 달라도 본 가이드의 좌표/각도 규약과는 무관. |
| `vite` | **7.x** | 빌드/HMR. |
| `electron` | **35.x** | 데스크톱 패키징(좌표 규약과 무관). |
| `nx` | **22.5.4** | 모노레포 태스크 러너. |
| `typescript` | **5.9.x** | `DragState` 유니온 등 타입 규약. |

- [ ] 그룹 프로젝트의 `maplibre-gl`이 **4.x(특히 4.7.1)** 인지 확인. **v5면** custom layer가 `render(gl, args)` + `args.defaultProjectionData.mainMatrix`로 바뀌어 Part 3 / 9장 매트릭스 처리가 통째로 달라짐 → 이 경우 도형이 안 보이거나 어긋나는 1순위 원인.
- [ ] 그룹 프로젝트도 **MapLibre의 Mercator Y-down** 좌표를 쓰는지 확인(같은 4.7.1이면 동일). 이게 1·3·4·8-A장 Y부호/각도 규약의 출발점.
- [ ] `three` 메이저가 같은 0.17x~0.18x 계열인지 확인. `Matrix4`/`MathUtils` API는 이 범위에서 동일(이 저장소는 **0.174.0**).

---

## 분석 대상 프로젝트의 현재 구조 (제보 기반) — 핵심 의심 지점

> 제보된 그룹 도형 프로젝트의 현재 아키텍처:
> - **screen 이동**: 드래그 시작 시점을 **snapshot**으로 떠서 그 위에서 평행이동(snapshot 기반)
> - **map 편집의 guide·bbox(선택상자)**: 주로 **screen 좌표** 기준으로 계산
> - **실제 회전 시 도형 갱신**: **도형별(per-shape) map 로직**이 섞여 있음
>
> → 즉 **선택상자/가이드(screen 프레임)** 와 **실제 도형 갱신(map 프레임)** 이 **서로 다른 좌표 프레임**에서 돈다.
> 이게 8-A "측정 프레임 ≠ 렌더 프레임 → Y반사 홀수 → 각도 반전 / wrap"의 **구조적 원인**과 정확히 일치한다.

이 구조가 맞는지, 그리고 그게 버그를 만드는지 항목별로 점검:

- [ ] **프레임 불일치**: 회전 각도를 **재는 곳(guide/bbox=screen)** 과 **적용·렌더하는 곳(도형 갱신=map)** 의 좌표 프레임이 다른가? → 다르면 Y반사 횟수가 갈려 **각도 반전(증상①)** 의 직접 원인. (8-A 증상① / "반사 홀짝 세는 법" 참고)
- [ ] **혼합 로직**: "도형별 map 로직"이 섞여 있어 **도형 종류마다 Y반사/회전 부호 처리가 다른가?** → 일부 도형만 반대로 돌거나 사각형에서만 `-angle`이 필요한 이유일 수 있음.
- [ ] **bbox vs 실제 도형 어긋남**: screen 기준 bbox(선택상자)와 map 기준 실제 도형이 **회전 시 서로 벌어지는가?** (특정 각도 이상에서 선택상자와 도형이 안 겹침) → 5·9장 "동일 변환 공유" 위반.
- [ ] **snapshot vs map 중심 불일치**: snapshot 기반 screen 이동의 기준점과 map 회전의 중심(피벗)이 **같은 좌표를 공유하는가?** → 어긋나면 이동 후 회전에서 점프/드리프트.
- [ ] **measure→apply 일관성**: 한 동작(회전) 안에서 **measure(각도 측정)·guide(가이드 표시)·update(도형 갱신)** 가 **모두 같은 프레임**을 쓰는가? 셋 중 하나라도 프레임이 다르면 그 지점이 버그.

> **권장 교정 방향**: 측정·가이드·갱신을 **하나의 프레임으로 통일**한다.
> - screen에서 재면 screen에서 적용, **map(로컬 미터)에서 재면 map에서 적용** — 섞지 않는다.
> - 이 레퍼런스(`map-aabb`)는 **모두 로컬 미터 프레임**으로 통일해서 guide(SVG 오버레이)·실제 도형(Three 메쉬)이 같은 `projectLocalPoint`/`getPointerLocalMeters`를 공유한다(5·9·10장). 그래서 `-angle` 같은 모드/도형별 보정이 전혀 없다.

### 재현 데모 — 우리 프로젝트에 같은 버그를 심어둠 (before/after = 수정안)

> 그쪽에서 "어떻게 고칠지"가 바로 나오도록, 보고된 회전 버그를 **우리 프로젝트(`map-aabb`)에 동일하게 재현**해 두었다.
> 초기 도형을 **사각형**으로 바꾸고, 회전 핸들 로직에 그쪽 구조를 그대로 넣었다.

- [ ] **토글 플래그**: `map-aabb/src/app/app.tsx`의 `REPRODUCE_GROUP_PROJECT_BUGS` (`app.tsx:16` 부근).
  - `true` → **버그 재현**: ① rectangle map-mode **부호 반전**(`measured = -rawAngleDeg`) → 회전 방향이 포인터와 반대, ② 로컬 회전 **delta 누적 + 단순 `%360`**(이중 모듈로 아님) → ±180° 분기점에서 "일정 이상 돌리면 반대로".
  - `false` → **정상(수정안)**: 로컬 Y-up 단일 프레임에서 **절대각 set + `normalizeDegrees`(이중 모듈로)**, 부호 반전·delta 누적 없음.
- [ ] **그쪽 프로젝트 수정 = 이 플래그를 끈 diff**. 재현 분기(`if (REPRODUCE_...)`)를 제거하고 정상 분기만 남기면 된다. (해당 코드: 회전 핸들러 `app.tsx:709~735` 부근)
- [ ] **시연 순서**: 플래그 `true`로 두고 사각형 회전 핸들을 돌려 ①(반대 방향)·②(경계서 튐)를 확인 → 플래그 `false`로 바꿔 같은 동작이 정상이 되는지 확인.

---

# Part 1 — 그룹 도형 프로젝트 AI에게 던질 질문지

> **용도**: 아래 "붙여넣기용 프롬프트" 블록을 그대로 그쪽 AI에게 던져 **현재 구현 설계**를 받는다.
> 받은 답을 Part 2/Part 3와 대조해 우리가 수정한다.
...
**H. 프레임 구조 (가장 의심되는 부분 — 정확히 답해줘)**
16. screen 이동은 **snapshot 기반**인가? snapshot은 어떤 좌표(screen px / map mercator)로 떠지나?
17. map 편집의 **guide·bbox(선택상자)**는 **screen 좌표**로 계산하나, **map(mercator/local) 좌표**로 계산하나?
18. **실제 회전 시 도형 갱신**은 어느 프레임에서 하나? 도형 종류마다 **다른 로직(per-shape)**이 섞여 있나? (그렇다면 어떤 도형이 어떻게 다른지)
19. 회전 한 동작 안에서 **각도 측정 / guide 표시 / 도형 갱신**이 **같은 좌표 프레임**을 쓰나, 서로 다른가?

(여기까지 복사)

---

# Part 2 — 받은 답 해석 가이드 (수정 지점 매핑)

받은 답을 이렇게 해석해 수정 지점을 정한다. 상세 근거는 Part 3의 해당 장.

- **8·9·10·11 → angle 반전의 진짜 원인**: 반사가 홀수면 `-angle`이 "필요해 보이는" 구조. 답에서 Y반사 개수가 측정/렌더 간 다르면 → **각도 보정(`-angle`)을 떼고 좌표 측정 단계에서 Y를 한 번만 뒤집는** 방향으로 수정. (Part 3 / 8-A 증상①)
...
- **16·17·18·19 → 프레임 혼합(핵심)**: guide/bbox=screen인데 도형 갱신=map이면 **측정·적용 프레임 불일치** → 각도 반전·wrap·bbox 어긋남의 구조적 원인. → **측정·가이드·갱신을 한 프레임으로 통일**(레퍼런스는 전부 로컬 미터). 도형별 로직이 섞여 있으면 그 분기를 공통 변환으로 합침. ("분석 대상 프로젝트의 현재 구조" 절 + 8-A)

---

# Part 3 — 레퍼런스 구현 체크리스트 (대조 기준)

> 이 저장소(`map-aabb`)의 올바른 규약. 모든 항목은 `[ ]` 체크박스. 그룹 프로젝트 답/코드가 같은 규약을 지키는지 한 줄씩 대조하세요.
> 참조 줄번호는 모두 `map-aabb/src/app/app.tsx` 기준.
...
**"일정 이상 회전하면 반대로 가는" 현상 — 그쪽 구현 점검 체크리스트** (각 항목이 그룹 프로젝트에서 참인지 확인. 하나라도 ❌면 그게 원인):

- [ ] 회전값을 **매 프레임 포인터의 절대 각도로 직접 set** 한다. (`rotation = atan2(...)` 형태)
- [ ] 회전값을 **이전 프레임 대비 delta로 누적하지 않는다.** (`rotation += Δ`, `rotation = prev + (cur-start)` 형태가 **없어야** 함)
- [ ] 저장 직전 각도를 **`((v+180)%360+360)%360-180` 형태(이중 모듈로)로 정규화**한다. (단순 `v%360` 아님)
- [ ] `atan2` 입력으로 쓰는 좌표가 **회전 중심(그룹/도형 중심) 기준**이다. (자식·핸들 로컬이 아니라 피벗 기준)
- [ ] `atan2`에 넣는 Y가 **Y 반전을 거친 로컬 좌표**다. (8-A 증상① Y부호 항목과 짝)
- [ ] 핸들이 ±180° 경계를 넘을 때 각도를 **누적·clamp·`Math.min/max`로 가두지 않는다.** (가두면 그 지점에서 반대로 꺾임)
- [ ] (만약 굳이 delta를 쓴다면) delta 자체를 **`normalizeDegrees`로 감싼다.** (경계 점프 -360°를 흡수)

> 레퍼런스 근거: 위 7개를 모두 만족하는 게 `app.tsx`의 `normalizeDegrees`(124) + 회전 핸들러(700~703) 구현. **절대 각도 set + 이중 모듈로 정규화** 조합이라 ±180 분기점을 넘어도 도형이 반대로 튀지 않음.
