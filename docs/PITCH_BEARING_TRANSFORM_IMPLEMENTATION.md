...
높은 pitch 또는 타일 좌표계에서 마우스 이동 대비 factor가 과도하게 적용되는 현상은 `PITCH_BEARING_FACTOR_DIAGNOSIS.md`의 고정 번호 체크리스트를 사용한다. 다른 프로젝트에서는 `PB-F번호`와 공통 진단 로그를 전달하고, 이 프로젝트에서 같은 번호의 재현 절차로 원인을 분리 검증한다.
